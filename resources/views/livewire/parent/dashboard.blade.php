<div>
    ...
    @if (!$student)
        <div class="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800/50 rounded-2xl p-6 shadow-sm mb-8 flex items-start gap-4">
            ...
            <div>
                <h3 class="text-base font-semibold text-red-800 dark:text-red-400">No Linked Child</h3>
                <p class="text-sm text-red-700 dark:text-red-300 mt-1">
                    No student is currently linked to your parent account. Please contact the school administrator to link your account to your child's profile.
                </p>
            </div>
        </div>
    ...
    @elseif (!$enrollment)
        <div class="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/50 rounded-2xl p-6 shadow-sm mb-8 flex items-start gap-4">
            ...
            <div>
                <h3 class="text-base font-semibold text-amber-800 dark:text-amber-400">Child Not Enrolled</h3>
                <p class="text-sm text-amber-700 dark:text-amber-300 mt-1">
                    Your child, {{ $student->first_name }} {{ $student->last_name }}, is not enrolled for the current school year ({{ $activeSchoolYear->name }}). Please contact the academic office.
                </p>
            </div>
        </div>
    @else
        {{-- Premium Child Name Banner --}}
        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20 border border-blue-100 dark:border-blue-900/30 rounded-2xl p-5 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h2 class="text-[10px] font-bold text-blue-500 dark:text-blue-400 uppercase tracking-widest">Linked Student Profile</h2>
                <h1 class="text-xl font-bold text-zinc-900 dark:text-white mt-1">
                    {{ $student->first_name }} {{ $student->middle_name }} {{ $student->last_name }} {{ $student->suffix }}
                </h1>
                <p class="text-xs text-zinc-550 dark:text-zinc-400 mt-1.5 flex items-center gap-1.5">
                    <i class="ti ti-id-badge text-blue-500"></i>
                    LRN: <span class="font-bold text-zinc-700 dark:text-zinc-350">{{ $student->lrn ?? 'N/A' }}</span>
                </p>
            </div>
            <div class="flex items-center gap-2 shrink-0">
                <span class="inline-flex items-center px-3 py-1 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-sm text-xs font-semibold text-zinc-700 dark:text-zinc-300">
                    <i class="ti ti-users mr-1.5 text-indigo-500"></i>
                    Parent Portal Mode
                </span>
            </div>
        </div>
                ...
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden">
                    <div class="px-6 py-5 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div class="flex items-center justify-between w-full sm:w-auto gap-4">
                            ...
                            <a href="{{ route('parent.grades.index') }}" class="text-xs text-blue-600 hover:underline dark:text-blue-400 font-medium">
                                View all years
                            </a>
                        </div>
                    ...
                    <div class="p-0">
                        @if ($subjects->isEmpty())
                            <div class="flex flex-col items-center justify-center text-zinc-400 py-12">
                                ...
                                <p class="text-xs mt-1 text-zinc-500">There are no subjects assigned to your child's section for this school year.</p>
                            </div>
                        @else
                                    ...
                                    <div class="bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-800 rounded-xl p-4 transition-all duration-200 relative overflow-hidden group hover:border-zinc-200 dark:hover:border-zinc-700 {{ $announcement->is_pinned ? 'border-l-4 border-l-amber-400 dark:border-l-amber-400' : '' }}">
                                        ...
                                        <div class="flex justify-between items-start gap-2 mb-1.5">
                                            ...
                                            <button 
                                                ...
                                                class="text-zinc-400 hover:text-zinc-650 dark:hover:text-zinc-200 shrink-0 p-0.5 rounded hover:bg-zinc-200/50 dark:hover:bg-zinc-700/50 transition-colors cursor-pointer"
                                        ...
                                        <div class="flex items-center gap-2 mb-3 text-[10px] text-zinc-400 dark:text-zinc-500">
                                            ...
                                            @else
                                                <span class="px-1.5 py-0.5 text-[9px] font-bold rounded bg-pink-50 dark:bg-pink-950/20 text-pink-700 dark:text-pink-400 border border-pink-100 dark:border-pink-950/40">
                                                    Parent
                                                </span>
                                            @endif
                                        </div>
